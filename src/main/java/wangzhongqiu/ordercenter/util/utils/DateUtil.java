...
public class DateUtil {
    ...
    /**
     * 将String类型的时间转换为Date型 支持格式有：yyyyMMdd
     */
    ...
    public static Date parse(String time, String pattern) {
        try {
            ...
            throw new IllegalArgumentException(
                    "Parse Date Error: " + time + ", With Pattern: " + pattern);
        }
    }
    ...
    public static String getDateByTimeStamp(long timestamp) {
        ...
        String
                date =
                new SimpleDateFormat("EEE, dd-MMM-yyyy HH:mm:ss").format(new Date(timestampMs));
        return date;
    }
    ...
    /**任务用 开始**/
    public static Date nextTime(Date date, int number,int coefficient) {

        Double random = Math.random()*5;

        int addRandom = random.intValue();

        Calendar calendar = Calendar.getInstance();

        calendar.setTime(date);

        calendar.add(Calendar.SECOND, number * number * coefficient * 5 + addRandom);

        return calendar.getTime();
    }

    public static Date nextTimeForDelay(Date date,int delayTime) {

        Calendar calendar = Calendar.getInstance();

        calendar.setTime(date);

        calendar.add(Calendar.HOUR, delayTime);
     ...
     * @return
     */
    public static Date plusDay(int dayNum,Date date){
